...
public class ServerLoginPacketListenerImpl implements ServerLoginPacketListener {
   ...
   public void handleAcceptedLogin() {
      ...
      if (component != null) {
         ...
         try {
            ServerPlayer serverplayer1 = this.server.getPlayerList().getPlayerForLogin(this.gameProfile, this.playerProfilePublicKey);
            if (serverplayer != null) {
   ...
   @Nullable
   private static ProfilePublicKey validatePublicKey(ServerboundHelloPacket p_215257_, SignatureValidator p_215258_, boolean p_215259_) throws ServerLoginPacketListenerImpl.PublicKeyParseException {
      try {
         Optional<ProfilePublicKey.Data> optional = p_215257_.publicKey();
         if (optional.isEmpty()) {
            if (p_215259_) {
            ...
            return ProfilePublicKey.createValidated(p_215258_, optional.get());
         }
      } catch (InsecurePublicKeyException.MissingException missingexception) {
         if (p_215259_) {
            throw new ServerLoginPacketListenerImpl.PublicKeyParseException(INVALID_SIGNATURE, missingexception);
         } else {
            return null;
         }
      } catch (CryptException cryptexception) {
         throw new ServerLoginPacketListenerImpl.PublicKeyParseException(INVALID_PUBLIC_KEY, cryptexception);
      } catch (Exception exception) {
         throw new ServerLoginPacketListenerImpl.PublicKeyParseException(INVALID_SIGNATURE, exception);
      }
   }
   ...
   public void handleHello(ServerboundHelloPacket p_10047_) {
      ...
      try {
         this.playerProfilePublicKey = validatePublicKey(p_10047_, this.server.getServiceSignatureValidator(), this.server.enforceSecureProfile());
      } catch (ServerLoginPacketListenerImpl.PublicKeyParseException serverloginpacketlistenerimpl$publickeyparseexception) {
         LOGGER.error(serverloginpacketlistenerimpl$publickeyparseexception.getMessage(), serverloginpacketlistenerimpl$publickeyparseexception.getCause());
         if (!this.connection.isMemoryConnection()) {
            this.disconnect(serverloginpacketlistenerimpl$publickeyparseexception.getComponent());
            return;
         }
      }
   ...
   public void handleKey(ServerboundKeyPacket p_10049_) {
      ...
      try {
         ...
         if (this.playerProfilePublicKey != null) {
            if (!p_10049_.isChallengeSignatureValid(this.nonce, this.playerProfilePublicKey)) {
               throw new IllegalStateException("Protocol error");
            }
         } else if (!p_10049_.isNonceValid(this.nonce, privatekey)) {
            throw new IllegalStateException("Protocol error");
         }
   ...
   static class PublicKeyParseException extends ThrowingComponent {
      public PublicKeyParseException(Component p_215261_) {
         super(p_215261_);
      }

      public PublicKeyParseException(Component p_215263_, Throwable p_215264_) {
         super(p_215263_, p_215264_);
      }
   }
