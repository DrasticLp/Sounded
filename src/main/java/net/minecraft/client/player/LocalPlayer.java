...
public class LocalPlayer extends AbstractClientPlayer {
   ...
   public void chat(String p_108740_) {
      this.chat(p_108740_, (Component)null);
   }

   public void chat(String p_234126_, @Nullable Component p_234127_) {
      MessageSigner messagesigner = MessageSigner.create(this.getUUID());
      this.sendChat(messagesigner, p_234126_, p_234127_);
   }

   public void command(String p_234157_) {
      this.command(p_234157_, (Component)null);
   }

   public void command(String p_234149_, @Nullable Component p_234150_) {
      MessageSigner messagesigner = MessageSigner.create(this.getUUID());
      this.sendCommand(messagesigner, p_234149_, p_234150_);
   }

   private void sendChat(MessageSigner p_234142_, String p_234143_, @Nullable Component p_234144_) {
      if (p_234144_ != null) {
         MessageSignature messagesignature = this.signMessage(p_234142_, p_234144_);
         this.connection.send(new ServerboundChatPacket(p_234143_, messagesignature, true));
      } else {
         MessageSignature messagesignature1 = this.signMessage(p_234142_, Component.literal(p_234143_));
         this.connection.send(new ServerboundChatPacket(p_234143_, messagesignature1, false));
      }

   }

   private MessageSignature signMessage(MessageSigner p_234146_, Component p_234147_) {
      try {
         Signer signer = this.minecraft.getProfileKeyPairManager().signer();
         if (signer != null) {
            return p_234146_.sign(signer, p_234147_);
         }
      } catch (Exception exception) {
         LOGGER.error("Failed to sign chat message: '{}'", p_234147_.getString(), exception);
      }

      return MessageSignature.unsigned();
   }

   private void sendCommand(MessageSigner p_234152_, String p_234153_, @Nullable Component p_234154_) {
      ParseResults<SharedSuggestionProvider> parseresults = this.connection.getCommands().parse(p_234153_, this.connection.getSuggestionsProvider());
      ArgumentSignatures argumentsignatures = this.signCommandArguments(p_234152_, parseresults, p_234154_);
      this.connection.send(new ServerboundChatCommandPacket(p_234153_, p_234152_.timeStamp(), argumentsignatures, p_234154_ != null));
   }

   private ArgumentSignatures signCommandArguments(MessageSigner p_234138_, ParseResults<SharedSuggestionProvider> p_234139_, @Nullable Component p_234140_) {
      Map<String, Component> map = ArgumentSignatures.collectLastChildPlainSignableComponents(p_234139_.getContext());
      if (map.isEmpty()) {
         ...
         if (signer == null) {
            ...
            try {
               ...
               map.forEach((p_234135_, p_234136_) -> {
                  Component component = p_234140_ != null ? p_234140_ : p_234136_;
                  MessageSignature messagesignature = p_234138_.sign(signer, component);
                  builder.put(p_234135_, messagesignature.saltSignature().signature());
               });
               return new ArgumentSignatures(p_234138_.salt(), builder.build());
            } catch (Exception exception) {
