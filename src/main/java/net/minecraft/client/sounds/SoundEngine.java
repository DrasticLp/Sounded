...
public class SoundEngine {
    ...
    public SoundExecutor musicExecutor, soundExecutor;
    ...
    public void play(SoundInstance p_120313_) {
        this.play(p_120313_, this.soundExecutor);
    }

    public void play(SoundInstance p_120313_, SoundExecutor e) {
        if (this.loaded) {
                                    ...
                                    if (!flag3) {
                                        this.soundBuffers.getCompleteBuffer(sound.getPath()).thenAccept((p_194501_) -> {
                                            channelaccess$channelhandle.execute((p_194495_) -> {
                                                if (p_120313_ == Minecraft.getInstance().getMusicManager().currentMusic)
                                                    p_194495_.playMusic(p_194501_, true, null);
                                                else
                                                    p_194495_.play(p_194501_, true,null);
                                            });
                                        });
                                    } else {
                                        this.soundBuffers.getStream(sound.getPath(), flag2).thenAccept((p_194504_) -> {
                                            channelaccess$channelhandle.execute((p_194498_) -> {
                                                if (p_120313_ == Minecraft.getInstance().getMusicManager().currentMusic)
                                                    p_194498_.playMusic(null, false, p_194504_);
                                                else
                                                    p_194498_.play(null, false, p_194504_);
                                            });
                                        });
                                    }
